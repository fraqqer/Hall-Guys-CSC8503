...
void OGLShader::ReloadShader() {
	...
	for (int i = 0; i < (int)ShaderStages::SHADER_MAX; ++i) {
		if (!shaderFiles[i].empty()) { // if there is a shader loaded into the game
	...
	glGetProgramiv(programID, GL_LINK_STATUS, &programValid);
		...
		programValid == GL_TRUE ? std::cout << "Shader loaded!" << std::endl : std::cout << "This shader has failed" << std::endl;
	#endif
}
